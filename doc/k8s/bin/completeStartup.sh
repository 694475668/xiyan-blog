...
echo '=====开始部署监控工具metrics====='
kubectl apply -f ../yaml/metrics-server-deployment.yaml

echo '=====开始部署监控工具prometheus====='
kubectl apply -f ../yaml/prometheus.yaml

echo '=====开始部署监控工具grafana====='
kubectl apply -f ../yaml/grafana.yaml

sleep 30echo '=====监控工具正在进行初始化,请等待...====='
../config/wait-for-it.sh  --timeout=60  -- echo "=====监控工具已经准备就绪====="

echo '=====开始部署dashboard可视化工具====='
kubectl apply -f ../yaml/dashboard.yaml

echo '=====开始部署sentinel====='
kubectl apply -f ../yaml/sentinel.yaml

sleep 30


echo '=====开始部署zipkin====='
kubectl apply -f ../yaml/zipkin.yaml
...
kubectl apply -f ../yaml/admin-server.yaml
