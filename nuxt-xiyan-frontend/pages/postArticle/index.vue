  ...
  <div class="bookNote-list-content">
    <Row>
      <Col :xs="24" :sm="24" :md="24" :lg="17">
        <Form ref="article" :model="article" :label-width="labelWidth">
          ...
          <FormItem label="所属语言：">
            <Select v-model="article.type" style="width: 180px">
              <Avatar
  ...
  methods: {
    async updateArticle() {
      ...
      let res = await this.$axios.put(
        "/web/article/update/" + this.article.id,
        this.res,
        {
    ...
    async getArticle(id) {
      let res = await this.$axios.get("/web/article/by/" + this.article.id, {
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
      });
    ...
    async saveArticle() {
      ...
      //数据进行加密
      this.res.requestData = AESEncrypt(dataJson, aesKey);
      let res = await this.$axios.post("/web/article/add", this.res, {
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
      });
      if (res.success) {
        this.$Message.success({
