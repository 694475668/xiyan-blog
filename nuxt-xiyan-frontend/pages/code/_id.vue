  ...
  <div class="home-content" v-cloak>
    <Row>
      <Col :xs="24" :sm="24" :md="24" :lg="17">
        ...
        >
          <code-page-header :article="article"></code-page-header>
          <code-page-content>
            <article
...
<script>
import CodePageHeader from "@/pages/components/views/code/CodePageHeader";
import CodePageContent from "@/pages/components/views/code/CodePageContent";
import CodePageFooter from "@/pages/components/views/code/CodePageFooter";
...
export default {
  ...
  components: {
    "code-page-header": CodePageHeader,
    "code-page-content": CodePageContent,
    "code-page-footer": CodePageFooter,
    "tag-wall": TagWall,
    recommend: Recommend,
    personal: Personal,
    "side-toc": SideToc,
  ...
  async asyncData({ $axios, query }) {
    let [data1, data2] = await Promise.all([
      $axios.get("/web/code/by/" + query.id).then((res) => {
        return res;
      }),
      $axios.get("/web/code/browse/" + query.id).then((res) => {
        return res;
      }),
    ]);
    return {
