  ...
  <div class="book-info" v-if="bookDoubanInfo !== undefined">
    <panel :title="$t('book.bookInfo')" :titleAlign="'center'">
      <div slot="content" class="content">
        ...
        <div class="info">
          <p class="title">
            <a :href="bookDoubanInfo.alt" target="_blank">{{
              bookDoubanInfo.title
            }}</a>
          </p>
          <p class="desc">
            <span>{{ $t("book.author") }}：</span
            >{{ bookDoubanInfo.author.join(" ") }}
          </p>
          <p class="desc">
            <span>{{ $t("book.publisher") }}：</span
            >{{ bookDoubanInfo.publisher }}
          </p>
          <p class="desc">
            <span>{{ $t("book.publishDate") }}：</span
            >{{ bookDoubanInfo.publish_date }}
          </p>
          <p class="desc">
            <span>{{ $t("book.pages") }}：</span>{{ bookDoubanInfo.pages }}
          </p>
          <p class="desc">
            <span>{{ $t("book.rating") }}：</span>
            <Rate :allowHalf="true" :disabled="true"></Rate>
          </p>
          <p class="desc">
            <Tag
              type="border"
              v-for="tag in bookDoubanInfo.tags"
              :key="tag.name"
              class="border-tag"
              >{{ tag.name }}</Tag
            >
...
export default {
  ...
  methods: {
    ...
    formatLocalBookInfo(book) {
      ...
      bookInfo.tags = book.book_tags.split("，").map((tagName) => {
        return { name: tagName };
      });
