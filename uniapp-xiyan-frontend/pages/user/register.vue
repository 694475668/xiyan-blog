<template>
	<view class="box">
		<nav-bar :title="'注册'" :back="true" :leftText="'返回'"></nav-bar>
		...
		<view class="nx-form">
			<input class="nx-input" placeholder="请输入邮箱" v-model="register.username" />
			...
			<view class="nx-input" style="display: inline-flex;justify-content: center;align-items: center;">
				<input placeholder="请输入验证码" v-model="register.code" />
				<!-- 验证码组件 -->
				<u-verification-code ref="uCode" @change="codeChange"></u-verification-code>
				<u-button type="success" size="mini" shape="circle" :ripple="true" @click="getCode">{{tips}}</u-button>
			</view>
			<button class="nx-btn" @click="registerBtn">立即注册</button>
	...
	export default {
		...
		methods: {
			...
			async registerBtn() {
				...
				const res = await this.$request({
					url: this.$baseURL+"/auth/user/register",
					method: "POST",
					data: this.register
				})
				if (res.success) {
